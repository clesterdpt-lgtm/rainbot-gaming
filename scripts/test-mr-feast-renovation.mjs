...
check("26 mobile all-floor light budget", /const mobileCircuitBudget = state\.mobileRenderProfile/.test(lightRendering) && /mobileCircuitBudget[\s\S]*?light\.userData\.fixtureRole === "primary"/.test(lightRendering) && /!mobileCircuitBudget \|\| light === mobileCircuitLight/.test(lightRendering), "mobile startup must keep one real emitter per circuit on every floor instead of compiling the full main-floor light set");
check("26 mobile floor handover", /syncLightRendering\(state\.mobileRenderProfile \? undefined : "fade"\)/.test(mansion) && !/prewarmLightingPrograms/.test(mansion), "mobile floor changes must snap to their bounded layout without compiling every floor during startup");
...
check("26 mobile retained-light brightness", /MOBILE_CIRCUIT_INTENSITY_SCALE = 2/.test(mansion) && /mobileCircuitBudget[\s\S]*?light === mobileCircuitLight[\s\S]*?MOBILE_CIRCUIT_INTENSITY_SCALE/.test(lightRendering), "the one-light-per-circuit mobile budget is too dim to keep rooms readable");
