...
- Storm Run begins with the player held on the rear lawn facing Mr. Feast at the open back door while he explains the twelve-checkpoint race and elimination rule. He speaks each `3–2–1` count, calls `Run!` with the first destination, and announces the next destination after every collected checkpoint; the collection chime remains the immediate physical confirmation. The race interface no longer names a checkpoint, gives written directions, or shows standings. It retains only a compact Storm Run / checkpoint count / elapsed-time strip, making Mr. Feast's speech the authoritative route guidance. The camera releases facing west toward checkpoint one. The player, Mara, and Juniper then follow twelve ordered breadcrumb checkpoints spread across the existing grounds, including one inside the hedge maze. After the first marker, the route continues north through the formal garden's clear east lane, crosses its front edge around the fountain and beds, and reaches the front drive through a dedicated carriage-turn marker instead of doubling back toward the start. It then loops along the east grounds, through one maze pass, and around the rear lawn to the pool finish. Each active checkpoint has a tall blue beacon and a raised guide visible from the previous marker, so spoken directions always have a visible destination and the route never depends on following contestants. Mara runs at `2.4 m/s` and Juniper at `2.5 m/s`, leaving recovery room beneath the player's stamina-limited `3.75 m/s` sprint; the player must reach checkpoint twelve before Mara to survive and eliminate her.
- Lightning in Storm Run is a scare cue, never a gameplay hazard. Mr. Feast appears exactly twice: first among the east-front-lawn trees when the player leaves the driveway checkpoint and turns left toward checkpoint seven, then in a straight hedge-maze corridor. Each reveal is staged in measured deep shadow at readable size with clear line of sight. A dedicated scare sting and close-bolt profile fire with a nearly immediate sharp crack over a `1.7x` recorded thunder roll and illuminate the surrounding grounds more strongly than ordinary lightning. Mr. Feast is visible only while that brighter multi-pulse flash is active and vanishes as soon as darkness returns, always within `1.2s`, without damage, slowdown, knockback, or a time penalty. Ordinary ambient lightning keeps its existing timing, brightness, and volume.
