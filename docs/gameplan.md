...
- Feast Says uses six short, psychologically coercive movement/crouch commands mixed with commands that omit `Feast says`; the player must obey only the genuine instructions and beat Kip's score to remain in the show. Its trust prompt lets the player point to Mara, Kip, or Juniper through the existing left/forward/right controls, judging whether they obeyed rather than whom they chose. Contestants visibly crouch, sidestep, backpedal, and point instead of sliding through the responses. A win eliminates Kip and reopens free investigation, while a player loss is an `ELIMINATED` game-over state with load/restart recovery.
