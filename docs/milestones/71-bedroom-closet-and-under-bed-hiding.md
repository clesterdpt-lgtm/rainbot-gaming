...
- [x] Follow-up feedback widened and raised the under-bed side interaction to a `0.34m` width, `0.72m` height, and `1.45m` depth centered `0.42m` above the floor, so the prompt is easy to acquire across the bed frame without changing the physical gap, camera anchor, or exit pocket. — test: `scripts/test-mr-feast-bedroom-hiding.mjs::source contract and under-bed lifecycle`
