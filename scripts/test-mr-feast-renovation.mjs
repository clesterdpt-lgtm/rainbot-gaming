...
check("44 front garden connection", /west-lawn-front-garden-connector[^;]*w:\s*12\.0[^;]*d:\s*garden\.pathWidth[^;]*x:\s*-19\.25[^;]*z:\s*garden\.frontJunctionZ/.test(yardBuild) && /formal-garden-front-approach[^;]*d:\s*frontApproachDepth[^;]*x:\s*garden\.centerX[^;]*z:\s*frontApproachCenterZ/.test(yardBuild), "front carriage path does not make a continuous L-shaped connection to the garden spine");
check("44 rear garden connection", /garden-approach-path[^;]*w:\s*11\.0[^;]*d:\s*garden\.pathWidth[^;]*x:\s*-19\.75[^;]*z:\s*garden\.rearJunctionZ/.test(yardBuild), "rear terrace and garden spine do not share a clean aligned junction");
check("44 derived garden endpoints", /const gardenFrontEndZ = garden\.centerZ \+ garden\.depth \/ 2;/.test(yardBuild) && /const frontApproachCenterZ = \(gardenFrontEndZ \+ garden\.frontJunctionZ\) \/ 2;/.test(yardBuild) && /const frontApproachDepth = garden\.frontJunctionZ - gardenFrontEndZ \+ 0\.4;/.test(yardBuild), "front garden approach is not derived from the garden endpoint and can drift out of alignment");
check("44 layout-driven garden", /const garden = YARD_LAYOUT\.garden;[\s\S]*?const gardenX = garden\.centerX;[\s\S]*?const gardenZ = garden\.centerZ;/.test(yardBuild) && /formal-garden-path-long[^;]*d:\s*garden\.depth[^;]*x:\s*gardenX[^;]*z:\s*gardenZ/.test(yardBuild), "formal garden geometry can drift away from its shared layout datum");
...
check("cache key", cacheKey === "20260715-west-garden-rear-alignment-2", `mansion page cache key is stale (${cacheKey || "missing"})`);
