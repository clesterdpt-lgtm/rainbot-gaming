...
- One recoverable camera-security event only when the existing camera cone and line-of-sight loop observes the active beam. Switching on alone never fabricates a source. A per-camera observation latch prevents repeated alerts from one continuous sighting, while a genuine sight break and short cooldown allow the same or another camera to alert again without requiring an off/on toggle. The event records the source and player position, flashes that camera's warning treatment, and sends Mr. Feast through the existing responding, searching, returning, and patrol lifecycle without directly starting pursuit or permanent lockdown.
...
- [x] Switching the flashlight on creates no alert until the camera loop actually observes it. A camera that sees the active beam creates exactly one `flashlight-use` event for that continuous observation, including when the player switched it on earlier behind an occluder and emerges without toggling again. After a genuine sight break and the `1.4s` cooldown, the same camera can report the still-on beam again. Each event records its camera and position, triggers a readable warning pulse, and moves Mr. Feast into the existing bounded response without starting pursuit, permanent lockdown, or frame-by-frame alert spam. — test: `scripts/test-mr-feast-flashlight.mjs::camera-owned observation and reacquisition`
