...
- Feast Says uses six short movement/crouch commands mixed with commands that omit `Feast says`; the player must obey only the genuine instructions and beat Kip's score to remain in the show. A win eliminates Kip and reopens free investigation, while a player loss is an `ELIMINATED` game-over state with load/restart recovery.
