...
Write and run `scripts/test-mr-feast-opening-welcome.mjs` red before implementation, then green after implementation. Capture the basement warning on desktop and the final threat on mobile, inspect `window.render_game_to_text()` throughout the sequence, and rerun both mansion regression suites plus the adjacent host-speech and player-system suites.
