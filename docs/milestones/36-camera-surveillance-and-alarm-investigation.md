...
- Add a data-driven network of visibly powered surveillance cameras across major rooms, halls, basement work areas, and exterior approaches while preserving bathrooms, closets/hiding spaces, and the deepest maze routes as intentional blind spots.
...
- Give the player fair suspicion/alarm feedback through camera LEDs, a compact HUD meter, and restrained audio cues.
...
- [x] Every indoor unit is mounted at the center of a wall rather than in a corner, one-way sweeps take at least ten seconds, and the local LED stays green while scanning, gives three red warning pulses during hostile acquisition, then turns solid red and follows lateral player movement before the alarm threshold. — test: `scripts/test-mr-feast-camera-security.mjs::wall-center slow sweep and warning tracking`
- [x] A player can cross a camera lane while its head faces away, while sustained unobstructed exposure in its cone fills a fair suspicion meter and detects the player after the configured grace period. — test: `scripts/test-mr-feast-camera-security.mjs::facing-away traversal and facing-toward exposure`
- [x] Camera policy permits ordinary show-space filming, immediately alarms on tagged sabotage performed in view, treats the unlocked basement as restricted, and enters global lockdown after any alarm or patron-feed sabotage. Blind sabotage starts lockdown without summoning Mr. Feast until a later sighting. — test: `scripts/test-mr-feast-camera-security.mjs::show restricted and lockdown policy`
...
- [x] Camera bodies, status feedback, and the suspicion HUD remain readable on desktop and 390×844 touch layouts with no new console errors. — test: `scripts/test-mr-feast-camera-security.mjs::desktop and mobile presentation`
...
- Automated proof covers 35 cameras: 27 wall-centered indoor units and eight outdoor chokepoints. Real-browser QA includes a 10.5–14.5 second one-way sweep, three-pulse warning and pre-alarm tracking, an actual keyboard-driven blind-side crossing, natural foyer LOS, a naturally blocked basement partition, continuous-alarm latching, full Mr. Feast response/return, unchanged light topology, and desktop/mobile captures under `output/playwright/mr-feast-camera-security/`.
