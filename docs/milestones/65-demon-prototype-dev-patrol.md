...
- The original regression failed red first on the missing `DEMON_PROTOTYPES` tuning table. After a user browser retained the older mansion runtime, the follow-up cache regression failed red on `20260725-energy-hud-bottom-hide-full-1`. The latest locomotion contract then failed red on the old prototype asset identity `20260725-demon-locomotion-polish-1`; the page, runtime, and manifest now share `20260725-demon-locomotion-propulsion-1`, and the focused browser path uses the visible Developer Mode button.
...
- The first Pale Maw correction removed temporal flips but sampled an already twisted Meshy idle frame. Eight-phase inspection caught the persistent crossed right-hind chain before acceptance. The final idle, walk, and run instead derive their axes from the clean processed bind pose and use only twist-free directional quaternion swings. Walk gives every contact point at least a `0.492m` authored travel-axis sweep and run at least `0.535m`; all four propelling upper limbs move by at least `40°`, while the exported reports retain zero added axial knee twist. The runtime walk/run playback is now `1.0`, and eight-phase live-rig measurement proves every actual hand/foot sweep covers at least the root distance of its planted half-cycle, preventing body slide between contacts.
...
- Blender neutral turntables live in `output/iterate/demon-prototypes/blender-animation-propulsion-final/`; clean in-mansion idle/walk/run captures for both creatures live in `output/iterate/demon-prototypes/browser/`, and eight-phase front/side locomotion contact sheets live in `output/iterate/demon-prototypes/browser/propulsion-test/`.
