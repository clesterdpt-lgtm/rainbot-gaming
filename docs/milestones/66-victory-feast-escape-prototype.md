...
- A new navigation mesh, cross-floor Saint traversal, or a replacement for the mansion's authored response graph. The Saint may reuse that existing door-aware graph for same-floor physical detours.
...
- [x] Blocking UI pauses the called countdown, and an explicit save/load preserves its exact remaining time. The player begins the event by speaking with Mr. Feast in the Dining Room (E/touch on the host), not a mid-table prop. — test: `scripts/test-mr-feast-victory-feast.mjs::called timer, persistence, and physical report`
...
- The Saint-audio refinement failed red first on the missing `voicelowlong05` manifest contract. The prepared 44.1 kHz stereo Vorbis runtime asset now decodes as a `21.014s` buffer only after the finale call or visible Developer Mode Saint needs it. Real Chromium proves one looping source, a `0.0849` target gain at `12.023m` versus `0.3498` at `2.133m`, more than a threefold settled GainNode increase, distance-driven `1350–6200Hz` filtering, master-mute ownership, and immediate zero-target cleanup on finale save normalization. Mansion Audio, Demon Prototype static/live browser, Flashlight desktop/mobile, Breath Stealth, Banquet Loss, full Contestant 13 desktop/mobile, syntax, and `git diff --check` pass; Renovation retains only its unrelated `28 stairwell continuity` baseline.
- The Saint-navigation refinement failed red first on the missing full-width navigation contract. Real Chromium stages the Saint `10.012m` from a target across the solid Library/Foyer divider: direct clearance is false, a multi-node door detour is selected, two graph steps carry it through the real Library doorway, the now-clear lane switches to direct pursuit, and `13.328m` of physical travel ends at `0m` target distance. A second `9.949m` diagonal across the Dining Room table begins collider-blocked, uses `294` detour frames, travels `11.585m` around the broad furnishing, and likewise ends at `0m`. Both probes remain physical, leak no QA catch suppression, and preserve the ordinary finale cleanup. Focused desktop/mobile Victory Feast and syntax/whitespace checks pass under `20260729-saint-route-navigation-1`.
