...
- [x] After the flash, ordinary sprint, crouch, camera acquisition, Mr. Feast pursuit, and existing hiding remain authoritative. The Saint uses a small physical first-slice threat loop without teleporting. Existing hiding switches off the flashlight, blocks camera acquisition, prevents a hidden Saint catch, and lets Mr. Feast lose the player through the existing bounded window. — test: `scripts/test-mr-feast-victory-feast.mjs::escape, hide, and catch`
...
- [x] A camera that actually sees the emitting beam retains the existing camera-owned `flashlight-use` consequence, while a zero-output defect cannot fabricate a flashlight sighting. — test: `scripts/test-mr-feast-victory-feast.mjs::flashlight risk remains camera-owned`
