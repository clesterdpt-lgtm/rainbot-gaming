...
- [x] The active camera-aligned beam uses the tuned `84` intensity, `10.6m` reach, and focused `0.35rad` cone. It materially brightens a central Archive patch over the off state, falls away much faster at the edge, retains a dark periphery, stays below the overexposure ceiling, casts no shadow, shows no carried flashlight model, and changes light energy rather than shader-light counts. — test: `scripts/test-mr-feast-flashlight.mjs::brighter longer focused beam`
