...
- Keep both existing Workroom fixtures active inside the fixed shader-light budget, while retaining the stronger east server-bank emitter and switched emissive rack practicals so neither half of the room falls dark.
...
- [ ] Both the west operator/monitor half and east server bank are visibly readable from the wide Workroom view using the existing two real emitters plus switched task practicals, with no increase to the fixed shader-light count. — tests: `scripts/test-mr-feast-renovation.mjs::balanced Workroom lighting` and `scripts/test-mr-feast-workroom-security-hub.mjs::dual-side luminance`; final appearance verified by user playtest
...
- The follow-up dual-light pass reserves both original Workroom emitters before optional basement extras consume the fixed 11-point budget. The west operator region improved from `0.304` mean luminance / `0.07%` visible pixels to `20.367` / `47.21%`, while the server side remains bright at `52.970` / `62.09%`. Compare `output/iterate/2026-07-16-workroom-dual-lighting-before.png` and `output/iterate/2026-07-16-workroom-dual-lighting-after.png`.
