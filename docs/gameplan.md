...
- Storm Run sends the player, Mara, and Juniper through twelve ordered breadcrumb checkpoints spread across the existing grounds, including one inside the hedge maze. Each active checkpoint has a tall blue beacon and raised guide that is visible from the previous marker, so the route can be followed without relying on the contestants. Mara runs at `2.4 m/s` and Juniper at `2.5 m/s`, leaving recovery room beneath the player's stamina-limited `3.75 m/s` sprint; the player must reach checkpoint twelve before Mara to survive and eliminate her.
- Lightning in Storm Run is a scare cue, never a gameplay hazard. At three selected checkpoint approaches, Mr. Feast waits in a measured deep-shadow position no brighter than `0.12` exposure. The race scare uses a slightly longer multi-pulse flash than ordinary ambient lightning and a close, louder thunder profile, then removes him within `0.9s` without damage, slowdown, knockback, or a time penalty. Ordinary ambient lightning keeps its existing timing and volume.
