...
- Pool stair waterline fix (user-directed): the water plane stopped at z=−19.88, short of the entry stairs, so the submerged lower treads sat dry with a gap. Rather than a second plane (which showed a seam), the single plane is extended north to z=−18.40 (`makeEstatePoolWater(19.4, 12.725, pool.centerX, -24.7625, -0.39)`); the north wall, coping, deck pavers, and dry top step are all opaque and hide the overshoot, while the water stays visible through the open stair gap and laps the submerged steps as one continuous surface. The shader's uv aspect correction is now derived from `width/depth` so rain rings stay circular at the new ratio, and the south edge, all colliders, and pool routes are unchanged.
- Opening-welcome host lighting (user-directed): `MR_FEAST_OPENING_WELCOME.hostPosition.z` moved from `8.15` to `7.9`, halving Mr. Feast's distance to the foyer grand chandelier's downlight center (x=0, z=7.7) and pulling him back from the dim storm-lit doorway. Verified two ways: a live face-region pixel-luminance sample (average over a fixed box at his head, sampled from the actual rendered canvas) rose from 27.0 to 30.7 — a real ~13.5% brightness increase, not just a screenshot impression — and before/after screenshots at the same welcome line confirm it visually. The new position stays well inside both bounds the opening-welcome test pins: host-to-door 4.1 m (≤4.25 m cap) and host-to-player 2.3 m (1.2–4.5 m range). Note: `scripts/test-mr-feast-opening-welcome.mjs` itself currently cannot run — its `startWelcome()` waits for `getMrFeastState().loaded` *before* clicking Start, but character loading (`scheduleOptionalCharacterLoads`) only fires from inside the Start-click handler, so that wait never resolves. This is pre-existing (file untouched since the original welcome commit, unrelated to this change) and was worked around by replicating the suite's exact diagnostic assertions (movement/interaction lock, distance bounds, spoken-line tracking) against the corrected click-then-wait sequence instead.
