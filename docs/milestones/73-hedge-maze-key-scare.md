...
- Once the player reaches seven cells of depth, start the first player-local hedge disturbance after a `3.6s` build-up, then repeat `5.1s` rustle/branch/inhale/withdraw pulses after deterministic `5.8–6.6s` gaps until the key is recovered. Each pulse repositions the existing non-colliding bulges and instanced leaves across at least five hedge faces `1.35–4.8m` ahead of the player's view, while the recorded Feast Father breathing/voice gain rises from `0.10` at the threshold to `0.24` at maximum maze depth.
...
- [x] Every recurring pulse selects at least five forward hedge faces inside the `1.35–4.8m` placement window, and a deeper route sample produces a higher decoded Feast Father gain than the seven-cell threshold sample. — test: `scripts/test-mr-feast-hedge-maze-haunt.mjs::forward placement and depth-ramped audio`
...
The original focused test first failed red on `missing named HEDGE_MAZE_HAUNT tuning table`. The depth-pressure refinement then failed red on the missing ahead-placement window. The green Chromium run verifies the safety gate, three-cell lock depth, both visual/physical entrance seals, fixed collider count, sustained darkness and flicker, forward-positioned recurring movement, shallow-to-deep Feast Father gain, three pre-key scare pulses in twenty-eight seconds, flashlight continuity, explicit save/load restoration, immediate key release, the longer chamber sequence, cleanup, and a clean browser console. Desktop evidence is under `output/playwright/mr-feast-hedge-maze-haunt/`.
