...
async function run() {
  ...
  // Red-first source contract: the key remains authoritative and is granted
  // before this non-colliding, one-shot presentation can begin.
  assert(/const HEDGE_MAZE_KEY_SCARE = Object\.freeze\(\{/.test(runtime), "missing named HEDGE_MAZE_KEY_SCARE tuning table");
  ...
  assert(!/addFixedBox|addKinematicBox|createCollider/.test(scareClass), "the hedge scare must not create collision geometry");
  ...
  assert(/hedge-maze-key-scare/.test(html), "the runtime cache key should identify the hedge-maze key scare release");
  ...
  try {
    ...
    await page.evaluate(() => window.MrFeastFresh.advanceHedgeMazeKeyScareForQA(1.35));
    ...
    await page.evaluate(() => window.MrFeastFresh.advanceHedgeMazeKeyScareForQA(2.35));
    ...
    await page.evaluate(() => window.MrFeastFresh.advanceHedgeMazeKeyScareForQA(1.75));
    scare = await page.evaluate(() => window.MrFeastFresh.getHedgeMazeKeyScareState());
    ...
    console.log("Mr. Feast hedge-maze key scare acceptance passed: key-first trigger, one-shot persistence, non-colliding hedge movement, leaves, localized light/rain hush, directional inhale/retreat audio, Storm Run guard, cleanup, and clean browser console verified");
  } finally {
