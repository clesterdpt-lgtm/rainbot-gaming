...
- Storm Run begins with the player held on the rear lawn facing Mr. Feast at the open back door while he explains the twelve-checkpoint race and elimination rule. He speaks each `3–2–1` count, calls `Run!` with the first landmark, and names the next checkpoint after every collection; he no longer calls specific turns or compass directions. The collection chime remains the immediate physical confirmation. The race interface retains only a compact Storm Run / checkpoint count / elapsed-time strip. The camera releases facing west toward checkpoint one. The player, Mara, and Juniper then follow twelve ordered blue breadcrumbs across the formal garden, front grounds, east lawn, one hedge-maze pass, rear lawn, and pool finish. Each active checkpoint has a tall beacon and raised guide visible from the previous marker, so every short landmark call has a visible destination and the route never depends on following contestants. The north hedge-maze entry and first bend reuse two existing fixtures inside the fixed six-light exterior budget; their measured phone-route exposure stays at or above `0.282` without brightening the maze apparition's deliberate shadow. Mara runs at `2.4 m/s` and Juniper at `2.5 m/s`, leaving recovery room beneath the player's stamina-limited `3.75 m/s` sprint.
- Lightning in Storm Run is a scare cue, never a gameplay hazard. Mr. Feast appears exactly twice: first among the front-lawn trees after checkpoint six, then in a straight hedge-maze corridor. The first scare accepts both the player's natural diagonal toward checkpoint seven and the longer carriage-path approach, closing the small trigger gap that could let phone players miss it entirely. Each reveal remains staged in measured deep shadow at readable size with clear line of sight. A dedicated scare sting and close-bolt profile fire with a nearly immediate sharp crack over a `1.7x` recorded thunder roll and illuminate the surrounding grounds more strongly than ordinary lightning. Mr. Feast is visible only while that brighter multi-pulse flash is active and vanishes as soon as darkness returns, always within `1.2s`, without damage, slowdown, knockback, or a time penalty.
