...
check("48 css maximize", /function setMaximized\(/.test(mansion) && /Exit fullscreen/.test(mansion) && /applyMaximizedChrome\(true\)/.test(mansion) && /is-maxed/.test(mansion) && /rb-game-maxed/.test(mansion), "Maximize lacks CSS full-viewport chrome");
check("48 escape keeps maximize", /intentionalMaximizeExitUntil/.test(mansion) && /Fullscreen API is intentionally not used/.test(mansion) && !/requestFullscreen\(\{\s*navigationUI:\s*"hide"\s*\}\)/.test(mansion) && /event\.code === "Escape"/.test(mansion) && /setMenuOpen\(!state\.menuOpen\)/.test(mansion), "Escape while maximized still demaxes or still enters native Fullscreen API");
