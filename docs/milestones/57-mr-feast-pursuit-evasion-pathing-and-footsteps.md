...
- [ ] Losing sight or entering a hiding spot gives Mr. Feast enough time to pursue the stale clue before the bounded search/return, while never reading the hidden player's live position. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::meaningful stale-clue search`
- [ ] After an escaped pursuit enters its post-loss search, a new hostile basement-camera sighting or clear point-blank basement encounter immediately starts a fresh pursuit; the housekeeping cooldown never grants detection immunity. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::post-loss reacquisition`
