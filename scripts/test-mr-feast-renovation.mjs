...
check("52 Workroom server lighting", /"WORKROOM": \["workroom lights"\]/.test(lightingMap) && /const workroom = new LightCircuit\("workroom lights"/.test(lightingBuild) && count(lightingBuild, /workroom\.addFixture/g) === 2 && /serverLighting:[\s\S]*?intensityScale: 1\.55/.test(cameraSecurityConfig) && /serverFixture\.userData\.fixtureRole = "primary"/.test(lightingBuild) && /serverFixture\.userData\.workroomRole = "server-side"/.test(lightingBuild) && /workroom-server-task-light/.test(lightingBuild) && !/cold room lights/.test(lightingBuild), "the two-light Workroom circuit does not focus its budgeted east fixture and switched task practicals on the server bank");
