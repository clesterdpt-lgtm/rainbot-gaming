...
- A minimal race status strip showing only event, checkpoint count, and elapsed time. Mr. Feast's speech carries the briefing, countdown, landmark-only checkpoint names, and outcomes without calling specific turns or compass directions; chimes and physical beacons remain the route feedback.
...
- [x] From the start line and every collected checkpoint, Mr. Feast names only the next landmark—never a specific turn or compass direction—while its raised guide projects on-screen, remains readable through hedge occlusion, sits no more than 32 metres away, and emits a dedicated chime when collected. — test: `scripts/test-mr-feast-storm-run.mjs::landmark-only spoken breadcrumb chain`
...
- The rear-door start separates a north-facing briefing pose from the west-facing race release. Mr. Feast's rule line, spoken `3–2–1`, `Run!` call, and twelve short landmark names make speech authoritative without narrating turns. The minimal strip keeps only progress and time, while the existing sounds and physical beacons carry precise route feedback.
