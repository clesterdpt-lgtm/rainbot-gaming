...
- Verified visual proof: `output/playwright/mr-feast-basement-key-trail/library-shelf-book-subtle-desktop.png` shows clear spacing on both sides, the restrained pull, and the correctly ordered spine mark; `output/iterate/xiii-cross-closeup.png` and `xiii-cross-detail.png` confirm the mark reads as a fully crossed `X` scratched into flat damaged leather.
