...
- [x] Active Mr. Feast pursuit/investigation or an active finale Saint creates light scared breathing at full energy. Mere camera observation does not, and the loop stops immediately when pursuit ends.
...
- [x] The pursuit loop interpolates continuously from `0.72×` playback/`0.16` gain at full energy to `1.18×`/`0.78` at empty energy. Pursued hold release and forced gasp use local `breathloop01` once at `0.96×`/`0.68`; recorded sources never stack, and both retain the prior procedural fallback.
...
3. Use deterministic browser probes for all capacity endpoints, calm sprint silence, full/half/empty pursuit playback, immediate pursuit cleanup, gasp paths, hearing ranges, room/occlusion gates, and first-hear investigation.
