...
check("26 mobile upper light budget", /mobileUpperBudget[\s\S]*?light\.userData\.fixtureRole === "primary"/.test(lightRendering) && /!mobileUpperBudget \|\| light === mobileCircuitLight/.test(lightRendering), "mobile upper floor must keep one real emitter per circuit instead of compiling the full floor light set");
check("26 mobile floor handover", /syncLightRendering\(state\.mobileRenderProfile \? undefined : "fade"\)/.test(mansion) && /layouts\.filter\(\(layout\) => layout\.floors\.length === 1\)/.test(lightRendering), "mobile floor changes must avoid expensive cross-floor light unions");
check("26 mobile room light culling", /function mobileUpperCircuitNames\(roomLabel\)/.test(mansion) && /mobileUpperCircuits\.has\(circuit\.name\)/.test(lightRendering) && /roomChanged && state\.mobileRenderProfile && state\.currentFloor === "SECOND FLOOR"/.test(updateLocation), "mobile upper floor must retire shader lights hidden behind other rooms and refresh at room boundaries");
check("26 mobile room prewarm", /MOBILE_UPPER_PREWARM_ROOMS\.map\(\(room\) => \(\{ \.\.\.layout, room \}\)\)/.test(lightRendering), "mobile upper-room light-count variants are not prewarmed before exploration");
...
check("cache key", cacheKey === "20260712-mobile-upper-render-1", `mansion page cache key is stale (${cacheKey || "missing"})`);
