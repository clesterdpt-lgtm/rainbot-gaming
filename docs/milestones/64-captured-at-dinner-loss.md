...
- [x] Every seated Patron keeps both formal arms in a simple untwisted resting pose down beside the body, with no hand or forearm reaching across the table; every generated mask retains at least the approved full-face presentation scale so no undersized face plate exposes the shared head. — test: `scripts/test-mr-feast-banquet-loss.mjs::patron side-rest arm and mask readability`
