...
check("26 all-device light budget", /const boundedCircuitBudget = true/.test(lightRendering) && /boundedCircuitBudget[\s\S]*?light\.userData\.fixtureRole === "primary"/.test(lightRendering) && /!boundedCircuitBudget \|\| light === boundedCircuitLight/.test(lightRendering), "desktop and mobile must keep one real emitter per circuit instead of compiling the full floor light set");
...
check("26 mobile retained-light brightness", /MOBILE_CIRCUIT_INTENSITY_SCALE = 2/.test(mansion) && /state\.mobileRenderProfile[\s\S]*?light === boundedCircuitLight[\s\S]*?MOBILE_CIRCUIT_INTENSITY_SCALE/.test(lightRendering), "the one-light-per-circuit mobile budget is too dim to keep rooms readable");
