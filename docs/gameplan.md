...
- Feast Says uses a fixed sequence of six genuine commands: step left, step right, step back, crouch, point to the contestant you distrust most, then step toward the contestant you would eliminate. The first four use ordinary movement/crouch input; the distrust choice uses the player's look plus E or touch Interact, and the elimination choice reads actual world-direction movement toward a staged contestant. Each command lasts 7.5 seconds and each result lasts 6 seconds, giving an authored anxious or funny contestant line time to land before Mr. Feast cuts in with a warning. Mara, Kip, and Juniper make distinct psychological choices, including choices that target the player. Their crouch and sidestep responses use an isolated planted procedural pose layer that resets from a captured base and cannot accumulate, while backpedal and target-facing point gestures remain readable. Player-facing rules state only that the lowest score is eliminated; authored scoring makes Kip the first NPC elimination when the player survives. A player loss is an `ELIMINATED` game-over state with load/restart recovery.
...
- Storm Run sends the player, Mara, and Juniper through five ordered checkpoints spread across the existing grounds, including one inside the hedge maze. Contestants visibly run an authored route but remain slower than the player's maximum sprint; the player must reach checkpoint five before Mara to survive and eliminate her.
- Lightning in Storm Run is a scare cue, never a gameplay hazard. At selected checkpoint approaches a flash briefly reveals Mr. Feast near the course, then darkness removes him without damage, slowdown, knockback, or a time penalty.
