...
- A Ballroom film-production set in place of the old report sign: one proportional broadcast camera, two studio lights, and a boom microphone frame Mr. Feast while he waits at the contestant check-in point. The shared camera uses `0.72` scale and at least `2.25m` host clearance so it cannot dominate the staging. The event starts only when the player faces him and uses E or touch Interact; four contestant marks, staged cast positions, and the responsive minimal status strip remain part of the live competition.
...
- [x] The called-state Ballroom scene contains no report sign and instead stages a `0.72`-scale broadcast camera at least `2.25m` from Mr. Feast, two studio lights, a boom microphone, and the waiting host. Facing Mr. Feast and using E or touch Interact is the only way to begin; doing so stages all three loaded contestants, the player, and four visible contestant marks without permanent teleports or seating reservations. — test: `scripts/test-mr-feast-feast-says.mjs::Ballroom production call and staging`
...
- The 2026-07-21 production-staging refinement scales the shared broadcast camera to `0.72` and moves it to `2.259m` planar clearance from Mr. Feast. It remains recognizable in the film-set silhouette without blocking the host or crowding the interaction mark.
...
- The final Feast Says production-call framing is captured at `output/playwright/competition-staging/feast-says-camera-polish-desktop.png`; diagnostics report `cameraScale: 0.72` and `cameraDistanceFromHost: 2.259` with no gameplay or interaction change.
