...
check("26 explicit mobile stage height", /#mansion-stage\s*\{\s*height:\s*clamp\(320px,\s*calc\(100dvh\s*-\s*212px\),\s*650px\);\s*min-height:\s*0;/s.test(page), "phone layout needs a definite viewport-budgeted stage height for reliable canvas measurement");
...
check("28 base shader compilation precedes ready controls", !/renderer\.compile\(scene, camera\)/.test(lightRendering) && /setLoading\("Preparing the first frame",\s*96\)[\s\S]*?renderer\.render\(scene, camera\)[\s\S]*?state\.ready\s*=\s*true[\s\S]*?requestAnimationFrame\(animate\)/.test(initSequence), "startup can still advertise Start/Load before the base estate has rendered once");
...
check("43 Mr Feast lifecycle", /mrFeastNpc\s*=\s*new MrFeastWanderer\(\)/.test(initSequence) && /function startOptionalCharacterLoads\(\)/.test(mansion) && /void mrFeastNpc\?\.load\(\)/.test(mansion) && /scheduleOptionalCharacterLoads\(OPTIONAL_CHARACTER_LOADING\.postStartDelayMs\)/.test(mansion) && /animatedObjects\.push\(this\)/.test(mrFeastWanderer), "Mr. Feast is not constructed early, deferred until after the Start/Load transition, and registered with the update loop");
...
check("56 flashlight pickup", /Take flashlight/.test(flashlightSystem) && /basement-flashlight/.test(flashlightSystem) && /addInteractionTarget/.test(flashlightSystem) && /removeInteractionTarget/.test(flashlightSystem), "flashlight is not a one-time physical E/touch pickup");
...
check("56 flashlight beam", /new THREE\.SpotLight/.test(flashlightSystem) && /castShadow = false/.test(flashlightSystem) && /intensity = this\.state\.on/.test(flashlightSystem), "flashlight beam is not a shadow-free, energy-toggled spotlight");
check("56 flashlight light-only presentation", /intensity:\s*74\b/.test(flashlightConfig) && !/carried-flashlight-(?:body|head|lens)/.test(flashlightSystem), "flashlight must use the brighter tuning without drawing a carried model");
