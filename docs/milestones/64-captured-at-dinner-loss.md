...
- [x] Every seated Patron keeps both formal sleeves, forearms, and hands visibly raised to the tabletop beside the place setting, and every generated mask uses at least the approved full-face presentation scale so no undersized face plate exposes the shared head. — test: `scripts/test-mr-feast-banquet-loss.mjs::patron tabletop arm and mask readability`
...
Mr. Feast physically catches the player → the game cuts to a readable first-person Dining Room sacrifice tableau where the player can see their compact torso-only underwear silhouette and four proportional separated limbs lying flat on one unlabeled platter before the host, six uniquely masked formal Patrons surround the table with both hands visible at their place settings, Mr. Feast delivers the complete final-cut/sacrifice/supper line, and only then do Load last save and Start over appear.
