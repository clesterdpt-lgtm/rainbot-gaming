...
- **Persistence:** the player's manual mansion save keeps using the existing `window.RBGameSaves` adapter and `rainbot_game_save:mr-feast-mansion` versioned payload. Up to three distinct safe checkpoint autosaves use the mansion-local versioned key `rainbot_game_autosaves:mr-feast-mansion`, so they do not appear as extra games in shared save/profile/cloud enumeration. Transient actions, live threats/competitions, and Dev Mode are never checkpointed.
...
- **Banquet victim assets:** one selected rigged Meshy underwear body supplies both static runtime GLBs; Blender may separate the source into a compact torso-only body with no shoulder/leg attachments and four proportional, individually readable limbs laid on one horizontal platter, but must preserve visible source-derived anatomy, keep every detached piece grounded in one flat layer, center X/Z origins on their staging marks, ground Y at zero, export standard uncompressed glTF 2.0 for Three.js r128, and record rejected paid attempts rather than excluding them from credit totals.
