...
- [x] Standing, walking, and seated arm layers include the complete shoulder-to-hand chain; hands stay separated on their anatomical sides, standing wrists hang below the shoulders, and walking adds a bounded `0.105rad` opposing counter-swing while each signed palm normal remains aligned toward the torso by at least `0.70` throughout the swing. Kip's idle and eight sampled walk phases keep both hand joints within `70°` of bind, cap wrist twist at `66°` and bend at `60°`, and retain at least `78%` of the same reference-selected deformed wrist-ring thickness instead of collapsing the blended forearm/hand seam. — test: `scripts/test-mr-feast-seating-and-routes.mjs::coherent arm-chain, procedural swing, inward walking-palm, and deformed wrist cross-section probes`
- [x] Every seated contestant flexes both elbows through a relaxed `45–105°` range, keeps each wrist `0.24–0.36` of the way from hip to knee, points fingers down the same-side thigh instead of backward, and rolls both hands palm-down so the signed palm normal points toward the thigh surface by at least `0.65`. Rig-specific thigh-normal support (`0.14m` Mara, `0.15m` Kip, `0.21m` Juniper) keeps each wrist/fingertip plane flat within `0.025m`, while a deformed skinned-mesh probe verifies both hands remain `0.005–0.06m` above the actual visible thigh surface instead of tucking through or floating over it. — test: `scripts/test-mr-feast-seating-and-routes.mjs::seated arm support, signed palm-plane, longitudinal wrist, and deformed surface-gap probes`
