...
- [x] The first-person victim reads as one compact torso-only silhouette: no bulky shoulder sleeves, arm-like side attachments, split thigh/leg stubs, or raised surgical caps remain on the body asset. The detached-limb platter remains unchanged. — test: `scripts/test-mr-feast-banquet-loss.mjs::torso-only victim silhouette`
- [x] Every seated Patron keeps both formal sleeves/arms readable beside the place setting, and every generated mask uses at least the approved full-face presentation scale so no undersized face plate exposes the shared head. — test: `scripts/test-mr-feast-banquet-loss.mjs::patron arm and mask readability`
...
Mr. Feast physically catches the player → the game cuts to a readable first-person Dining Room sacrifice tableau where the player can see their compact torso-only underwear silhouette and all four separated limbs plated before the host, six uniquely masked formal Patrons surround the table, Mr. Feast delivers the complete final-cut/sacrifice/supper line, and only then do Load last save and Start over appear.
