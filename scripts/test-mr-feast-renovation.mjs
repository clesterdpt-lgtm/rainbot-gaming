...
check("48 escape menu", /function toggleGameAudio\(/.test(mansion) && /dom\.menuMusic\.addEventListener\("click"/.test(mansion) && /Music: On/.test(mansion) && /Music: Off/.test(mansion), "Escape menu music control is not wired to the audio system");
check("48 resume reclaims look", /function setMenuOpen\(/.test(mansion) && /requestPointerLock\(\)/.test(section("function setMenuOpen(open)", "function activateCurrentInteraction()")), "closing the Escape menu does not re-request pointer lock for camera look");
check("48 book keeps cursor hidden", /Keep pointer lock while reading/.test(mansion) && /is-book-open/.test(mansion) && /cursor:\s*none/.test(page) && !/releasePointerLock\(\);\s*\n\s*if \(dom\.bookTitle\)/.test(mansion), "opening a book still releases pointer lock or shows the system cursor");
