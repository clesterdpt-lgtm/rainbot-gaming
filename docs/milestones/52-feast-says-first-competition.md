...
- A Ballroom film-production set in place of the old report sign: one long-lens cinema/broadcast camera, two studio lights, and a boom microphone frame Mr. Feast while he waits at the contestant check-in point. The shared camera uses `0.72` scale and at least `2.25m` host clearance so it cannot dominate the staging; the Feast Says-only placement pulls it about `0.75m` back from the lineup. Its locked lens and both studio-light faces remain on the fixed player/contestant-lineup target through the live briefing, commands, and results. The event starts only when the player faces him and uses E or touch Interact; the four authored contestant positions stay logic-only, with no colored floor circles or action pads, and the responsive minimal status strip remains part of the live competition.
...
- [x] Mr. Feast's initial rule explanation immediately exposes a keyboard- and touch-accessible `Skip rules` control that begins round one without skipping any live command, verdict, contestant response, or score state. — test: `scripts/test-mr-feast-feast-says.mjs::immediate briefing-only skip`
...
- [x] The called-state Ballroom scene contains no report sign and instead stages a `0.72`-scale long-lens cinema/broadcast rig at least `2.25m` from Mr. Feast, two studio lights, a boom microphone, and the waiting host. The Feast Says pose is roughly `0.75m` farther from the player/contestant lineup than the shared pose, and its lens plus both fixture faces lock to that lineup once live. Facing Mr. Feast and using E or touch Interact is the only way to begin; doing so stages all three loaded contestants, the player, and four visible contestant marks without permanent teleports or seating reservations. — test: `scripts/test-mr-feast-feast-says.mjs::Ballroom production call and staging`
...
- [x] The production camera reads as a long-lens cinema/broadcast rig (narrow body, matte box, lens rails, rear battery, viewfinder/monitor, pan handle, and pedestal), remains at `0.72` scale and at least `2.25m` host clearance, and its Feast Says-only placement keeps the lens roughly `0.75m` farther from the fixed player/contestant lineup. Both studio-light faces stay locked on that same lineup through briefing, command, and result rather than disappearing or turning back toward Mr. Feast. — test: `scripts/test-mr-feast-feast-says.mjs::live production-camera framing`
- [x] Feast Says uses the same authored player/contestant positions and command input but renders no colored floor circles, lineup rings, or circular action pads during its call, briefing, commands, or results. — test: `scripts/test-mr-feast-feast-says.mjs::floor-marker-free presentation`
...
- The 2026-07-21 production-staging refinement uses a shared long-lens cinema/broadcast profile at `0.72` scale. Feast Says overrides only the shared placement to keep `2.25m+` host clearance while pulling the lens about `0.75m` / 2.45 ft farther from the contestant lineup; a fixed lineup target keeps both the live camera and the physical studio-light faces aimed away from Mr. Feast through briefing, commands, and results.

## Verification

- The 2026-07-22 briefing-skip refinement passed a focused 844×390 touch-browser probe: `Skip rules` measured 84.8×44 px and advanced directly from the untouched briefing into live command one with no resolved response or console error.
...
- The final live Feast Says framing is captured at `output/playwright/mr-feast-feast-says/feast-says-long-lens-player-lights-{desktop,mobile}.png`; diagnostics pin the `long-lens-cinema` profile and its matte box, rails, rear battery, viewfinder, monitor, pan handle, and pedestal, plus the static player/contestant-lineup target, a `≥0.94` lens-facing dot, and two player-facing fixture heads across briefing, command, and result with no gameplay or interaction change.
- The floor-marker-free refinement removes all Feast Says ring and circular action-pad meshes while retaining the hidden authored positions for staging and command input. The same full browser suite passes desktop and phone flow with `floorMarkers.lineupRingCount: 0` and `actionPadCount: 0`; refreshed proof is `feast-says-long-lens-player-lights-{desktop,mobile}.png`.
