...
Make Mr. Feast's chase readable, fair, and physically convincing. His footsteps must land on the grounded contacts of the shipped stalk/run animation, a player who breaks both personal sight and hostile camera tracking must stop broadcasting their live position, entering a hiding spot must end the chase after a meaningful but bounded search window, and a clearly visible same-floor player must be approached along a straight collision-aware line before the authored route graph is used as fallback.
...
- [x] Direct pursuit cannot leave an authored stair/ramp at a fixed height: while Mr. Feast is between floor planes he must retain the vertical route, stay centered on the stair lane, and reach the destination floor before level direct steering resumes. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::stair-grounded pursuit`
- [x] A same-floor railing or furniture collider between Mr. Feast and a visible player suppresses direct steering immediately and selects a collision-clear graph detour around the object; clear-room direct steering remains unchanged. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::railing detour`
...
- [x] Losing sight or entering a hiding spot gives Mr. Feast enough time to pursue the stale clue before the bounded search/return, while never reading the hidden player's live position. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::meaningful stale-clue search`
- [x] After an escaped pursuit enters its post-loss search, a new hostile basement-camera sighting or clear point-blank basement encounter immediately starts a fresh pursuit; the housekeeping cooldown never grants detection immunity. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::post-loss reacquisition`
- [x] Active pursuit and its post-loss search suppress the ordinary `Speak with Mr. Feast` interaction so the prompt cannot visually contradict his threat state. — test: `scripts/test-mr-feast-pursuit-evasion.mjs::threat prompt ownership`
...
The latest basement replay reproduced the complete failure chain: first hostile camera pursuit, broken tracking, premature loss, then ignored later camera and point-blank encounters while `pursuit.cooldownActive` still protected the aftermath. That flag was created to keep overdue housekeeping from hijacking the search, but had also been included in `updateTrespassWatch()` and therefore granted player-detection immunity. It now remains a housekeeping gate only. The stale-clue clocks are `12s` unseen and `6.5s` hidden, a second hostile basement camera immediately reacquires during the loss search, a later clear `1.9m` encounter reacquires with `trackingSource: "proximity"`, and both threat states suppress the contradictory ordinary conversation prompt. The focused test failed red at the old `6.67s` loss and again on the stale prompt, then passed with four planted stalk contacts, 20 run contacts, 102 direct-steering frames, both reacquisitions, and zero browser errors. Visual proof is `output/playwright/mr-feast-pursuit-evasion/post-loss-basement-reacquired.png`.

The stair/obstacle follow-up reproduced two separate defects. On the west grand-stair flight, direct pursuit was allowed while Mr. Feast's root was between floor planes, so he advanced at a fixed `y=3.5` instead of following the rising stair surface. Across the upper foyer, the direct lane checked only each tiny movement step and the graph could choose a first anchor across the railing, so he pushed into the guard instead of taking the front crosswalk. Direct pursuit now requires matching floor planes and a full character-width physics sweep; between-floor movement retains its authored vertical route, collision-blocked sight uses an actually reachable graph anchor, and generated same-floor shortcuts must pass the same physics clearance test. The red-first focused regression finishes the stair probe at `(-2.48, 3.929, 1.935)` with `0.000m` ramp error and zero direct-steering frames, while the railing probe reports `detourReason: "obstacle"`, takes a five-node route north, and also uses zero direct-steering frames. Runtime/test syntax, renovation invariants, focused pursuit/evasion, grounded gait, camera security, caught pursuit, stealth meter, and `git diff --check` pass. Live Chromium state and inspected screenshots are under `output/playwright/mr-feast-pathing/`.
