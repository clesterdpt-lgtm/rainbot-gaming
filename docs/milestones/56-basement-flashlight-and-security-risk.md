...
- A focused flashlight system with a camera-aligned, shadow-free spotlight, a small carried-body/lens treatment, restrained flicker, and named brightness, distance, cone, penumbra, and concealment tuning.
...
- One recoverable camera-security event per valid off-to-on activation when a basement camera has a plausible clear view. The event records the source and player position, flashes that camera's warning treatment, and sends Mr. Feast through the existing responding, searching, returning, and patrol lifecycle without directly starting pursuit or permanent lockdown.
...
- [x] The active camera-aligned beam materially brightens a central Archive patch over the off state, falls away much faster at the edge, retains a dark periphery, stays below the overexposure ceiling, casts no shadow, and changes light energy rather than shader-light counts. — test: `scripts/test-mr-feast-flashlight.mjs::restrained beam`
...
- [x] A valid camera-visible off-to-on activation creates exactly one `flashlight-use` security event, records its camera and position, triggers a readable warning pulse, and moves Mr. Feast into the existing bounded response without starting pursuit or permanent lockdown. Leaving it on or switching it off does not duplicate the event. — test: `scripts/test-mr-feast-flashlight.mjs::camera and host alert`
...
- Focused visual/stealth measurements: the Archive center cone gains `6.6` luminance while the sampled edge gains `0.0`; a dark motionless crouch falls from `96.0` concealment off to `71.5` on while standing remains visibility `1`. Proof is in `output/playwright/mr-feast-flashlight/flashlight-{pickup,beam-off,beam-on}-desktop.png` and `flashlight-touch-mobile.png`.
