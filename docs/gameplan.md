...
- Storm Run begins with the player held on the rear lawn facing Mr. Feast at the open back door while he explains the twelve-checkpoint race and elimination rule. He speaks each `3–2–1` count and releases the contestants with only `Run!`; he never announces checkpoint landmarks. The player finds the route visually, with the collection chime as immediate physical confirmation. The race interface retains only a compact Storm Run / checkpoint count / elapsed-time strip. The camera releases facing west toward checkpoint one. The player, Mara, and Juniper then follow twelve ordered blue breadcrumbs across the formal garden, front grounds, east lawn, one hedge-maze pass, rear lawn, and pool finish. Each active checkpoint has a tall beacon and raised guide visible from the previous marker, so the route never depends on speech or following contestants. The north hedge-maze entry and first bend reuse two existing fixtures inside the fixed six-light exterior budget; their measured phone-route exposure stays at or above `0.282` without brightening the maze apparition's deliberate shadow. Mara runs at `2.4 m/s` and Juniper at `2.5 m/s`, leaving recovery room beneath the player's stamina-limited `3.75 m/s` sprint. When the player wins, the ending restages at the back door: Mr. Feast announces Mara's elimination, Mara covers her face with both hands and says, `I kept count of everyone who didn’t come back. I never thought you’d have to count me.` Juniper walks through the rear terrace and central stairs to her Reading Room routine. Mr. Feast remains visibly anchored at the back door even if the nearby player steps inside; Mara disappears and he resumes patrol only after the player is both at least sixteen metres from the finish and no longer viewing the scene.
- Lightning in Storm Run is a scare cue, never a gameplay hazard. Mr. Feast appears at the three mapped moments: far in the northwest trees only after the player reaches the north garden/front-yard edge, far in the northeast trees when the player is close to the front door, then life-size immediately beyond the hedge-maze south turn. Crossing each proximity trigger arms that event for its bounded route-progress window; it fires only after Mr. Feast enters the real camera view, passes the three-dimensional `0.90` forward-dot gate, and has a clear torso sightline. Looking away, at the ground, or at the sky therefore cannot consume the one-shot, and turning just after crossing the small trigger still works. All three silhouettes stay at believable human scale and at their marked ground coordinates; the two distant reveals use a wider local lightning fill instead of oversized models, while the close maze reveal remains unchanged. Runners remain visible throughout the flash, and the active checkpoint stays present at briefly reduced glow so it does not steal focus. A dedicated scare sting and limited close-bolt profile use the trimmed `2.1x` recorded thunder roll with a brief `16dB` rain duck; the added white-noise crack is deliberately absent because it read as static on device speakers. If a mobile browser temporarily interrupts Web Audio, the close bolt waits for the next trusted key or touch gesture and then plays once instead of being lost. Mr. Feast is visible only while that brighter multi-pulse flash is active and vanishes as soon as darkness returns, always within `1.2s`, without damage, slowdown, knockback, or a time penalty. The local flash fill stays attached to the active render-light topology at zero intensity between strikes, allowing the briefing to warm its shaders instead of compiling a new point-light variant during the first reveal.
